...
import "../lib/verifications/src/abstracts/AttestationAccessControl.sol";
import {Attestation, AttestationVerifier} from "../lib/verifications/src/libraries/AttestationVerifier.sol";
import {IEAS} from "../lib/verifications/lib/eas-contracts/contracts/IEAS.sol";
import {Predeploys} from "../lib/verifications/src/libraries/Predeploys.sol";
...
contract Tornado is ReentrancyGuard {
    ...
    constructor(
        ...
        verifier = _verifier;
        indexer = IAttestationIndexer(_indexer);
        schemaUid = _schemaUid;
    }

    modifier onlyAttestation() {
        Attestation memory attestation = _getAttestation(msg.sender);
        AttestationVerifier.verifyAttestation(attestation, msg.sender, schemaUid);
        _;
    }

    function _getAttestation(address recipient) internal view returns (Attestation memory) {
        bytes32 attestationUid = indexer.getAttestationUid(recipient, schemaUid);
        return _eas.getAttestation(attestationUid);
    }

    function deposit(uint256 _commitment) external payable onlyAttestation nonReentrant {
    ...
    function withdraw(
        ...
        uint[2] memory c,
        uint[2] memory input
    ) external payable onlyAttestation nonReentrant {
